...
def das_script():
    ...
    '''
    Sell Limit in the middle of the spread with $200 risk and stop equal to the price field
    '''
    print("Bid=%.2f" % Bid)
    print("Ask=%.2f" % Ask)
    ...
    TrailPrice = Ask + Bid / 2  # can't test this in python as DAS evaluates left to right
    print("PriceRisk=%.2f" % TrailPrice)
    """ TrailPrice = STRING_ROUND2 """  # Here is a way to prevent python execution but include in DAS
    Price = StopPrice - TrailPrice  # need to use price field to access in next step
